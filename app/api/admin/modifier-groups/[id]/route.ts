...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { data, error } = await supabase.from("modifier_groups").update(payload).eq("id", params.id).select().single();
...
export async function DELETE(_: Request, { params }: { params: { id: string } }) {
  try {
    assertAdminRequest();
    const supabase = getAdminSupabaseClient();
    const { error } = await supabase.from("modifier_groups").delete().eq("id", params.id);
    if (error) throw error;
